...
- **Open idea (not yet built):** a PreToolUse hook that warns when deleting a `src/main` file matching lifecycle patterns (`implements ChatMemoryStore`, `@Produces`, scope/`@PreDestroy` annotations), making §8 deterministically enforced rather than aspirational.
