...
class NimModelGatewayTest {
    ...
    void chunksEmittedInOrderThenCompletion() {
        ...
        var gateway = new NimModelGateway(model);
        List<ChatMessage> history = List.of(new ChatMessage(ChatMessage.Role.USER, "hi"));

        List<String> collected = gateway.stream(history)
    ...
    void modelErrorPropagatesAsMultiFailure() {
        var model = new ScriptedStreamingChatModel(
                handler -> handler.onError(new IllegalStateException("nim down")));
        var gateway = new NimModelGateway(model);
        List<ChatMessage> history = List.of(new ChatMessage(ChatMessage.Role.USER, "hi"));
        ...
        assertEquals("nim down", thrown.getMessage());
    }
